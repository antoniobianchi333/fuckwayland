...
`wdotool/` is the second shared layer, and that is deliberate rather than accidental:
`wwmctl` and `wxprop` drive its window backends and its X11 wire client
(`wdotool/x11_mini.py`), `wxrandr` and `wmirror` use `wdotool/ctx.py` for the
exception class, `wdotool/stdio.py` for the exit-status rule and
`wdotool/procs.py` for detached children. That is why the zipapp of every tool
carries `fwcommon` and `wdotool` (see § [The single-file builds](#the-single-file-builds)).
...
**One detach protocol.** `wdotool/procs.py` is the whole of it, and both callers use
...
**One stdio rule, for all six tools.** `wdotool/stdio.py:flush_stdout(prog)` is the
