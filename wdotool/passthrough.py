...
#: what `scripts/build-pyz.sh` stamps into every zipapp, and what our own
#: scripts carry as a comment: the head sniff looks for it
MARKER = b"fuckwayland"
_SNIFF = (MARKER, b"wdotool", b"wwmctl", b"wxprop", b"wxrandr", b"warandr")
_HEAD_BYTES = 4096
...
def session_kind(tool: str | None = None, env=None) -> str | None:
    ...
    key = (name, tuple(e.get(k) for k in _ENV_KEYS),
...
def is_us(cand: str) -> bool:
    ...
    3. head sniff: a compiled binary is never us (we are pure Python), and one
       of our markers in the first 4 KiB is;
...
def _is_help_request(tool, args):
    """`--help`/`--version` (and xdotool's bare `help`/`version`), or nothing
    at all. With no original installed these keep our own output instead of
    exiting 127 -- a help request must never answer "not found"."""
    if not args:
        return True
    a = args[0]
    if tool == "xdotool" and a.lower() in ("help", "version"):
        return True
    ...
    if not body:
        return False
    if "help".startswith(body) or "version".startswith(body):
        return True
    return all(c in "hv" for c in body)     # xdotool -hv
